...
export default function NavbarItemForm({
  ...
  async function handleSubmit(e) {
    ...
    const apiUrl = initial?.id ? `/api/admin/navbar/${initial.id}` : '/api/admin/navbar';
    ...
    if (!json.success) { setError(typeof json.error === 'string' ? json.error : 'Failed to save.'); return; }
    onSave(json.data);
  }
        ...
        <ul className="nav nav-tabs mb-3">
          {activeLocales.map((loc) => (
            <li key={loc} className="nav-item">
              <button type="button" className={`nav-link py-1 px-3 ${activeTab === loc ? 'active' : ''}`} onClick={() => setActiveTab(loc)}>
                {loc.toUpperCase()}
      ...
      <div className="row g-2 mb-3">
          ...
          <div className="d-flex gap-3 mb-2">
            {['url', 'page'].map((type) => (
              <div key={type} className="form-check">
                <input className="form-check-input" type="radio" id={`lt-${type}`} value={type} checked={linkType === type} onChange={() => setLinkType(type)} />
                <label className="form-check-label small" htmlFor={`lt-${type}`}>{type === 'url' ? 'Custom URL' : 'Linked page'}</label>
              </div>
            ))}
          </div>
          {linkType === 'url' && (
            <input type="text" className="form-control form-control-sm" placeholder="https://… or /path" value={url} onChange={(e) => setUrl(e.target.value)} />
          )}
          {linkType === 'page' && (
            <select className="form-select form-select-sm" value={linkedPageId} onChange={(e) => setLinkedPageId(e.target.value)}>
              <option value="">— Select page —</option>
              {pageOptions.map((p) => <option key={p.id} value={p.id}>{p.label} (/{p.slug})</option>)}
            </select>
          )}
        </div>

        {/* Parent selector — show any item as a potential parent (except self) */}
        <div className="col-sm-6">
          ...
          <select className="form-select form-select-sm" value={parentId} onChange={(e) => setParentId(e.target.value)}>
            ...
            {(parentOptions ?? [])
              ...
              .map((i) => <option key={i.id} value={i.id}>{i.path}</option>)
            }
          </select>
        </div>
        ...
        <div className="col-sm-6 d-flex align-items-end">
          <div className="form-check form-switch mb-1">
            <input className="form-check-input" type="checkbox" id={`newTab-${initial?.id ?? 'new'}`} checked={openInNewTab} onChange={(e) => setOpenInNewTab(e.target.checked)} />
            <label className="form-check-label small" htmlFor={`newTab-${initial?.id ?? 'new'}`}>Open in new tab</label>
          </div>
        </div>
      </div>

      <div className="d-flex gap-2">
        <button type="submit" className="btn btn-sm btn-primary" disabled={saving}>{saving ? 'Saving…' : initial?.id ? 'Update' : 'Create'}</button>
